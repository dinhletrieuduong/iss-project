...
import gui.dialog.TablePermissionGranteeDialog;
...
public class ManageTablesPage extends ControlBase {
    @FXML
    ...
    private void updateTable(ActionEvent e) {
        ...
        if (selected != null) {
            var selectedTable = this.fieldTableName.getText();
            var selectedSchema = (User)this.tbvSchemas.getSelectionModel().getSelectedItem();
            var dialog = new ManageTableDialog(selectedSchema.getUsername(), selectedTable);
            dialog.showAndWait();
        }
    }
    ...
    private void selectItem(MouseEvent e) {
        ...
        var selected = tbvTables.getSelectionModel().getSelectedItem();
        if (selected != null) {
            var selectedTable = (Table)selected;
    ...
    private void reloadTableForce() {
        ...
        for (var table : tableModel.getTablesInSchema(lastSchemaChoice)) {
    ...
    private void reloadTable(MouseEvent e) {
        ...
        if (!lastSchemaChoice.equals(selectedSchema.getUsername())) {
            ...
            for (var table : tableModel.getTablesInSchema(selectedSchema.getUsername())) {
                tbvTables.getItems().add(table);
            }
        }
    }
